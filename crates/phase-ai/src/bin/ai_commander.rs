...
fn run(cli: CliArgs) -> i32 {
    ...
    // without a 3rd field's) deck payload. A bad default feed is process-fatal
    // here (matching every other startup-time validation in this file:
    // `parse_difficulty`, `parse_action_cap`, `parse_games_file`) — it's
    // resolved before any game starts, on either path, so there is no partial
    // batch progress to preserve by treating it any more gently.
